...
controller.filter = async function (req, res) {
    ...
    try {
        ...
        console.log(error);
        return response_management.custom_response(res, error.statusCode ?? 404, new JsonFunctions().IsJsonString(error.error) ? JSON.parse(error.error) : JSON.parse(`{ "Message": "${error.error.toString()}" }`));
    }
}
...
controller.filter_post = async function (req, res) {
    ...
    try {
        ...
        console.log(error);
        return response_management.custom_response(res, error.statusCode ?? 404, new JsonFunctions().IsJsonString(error.error) ? JSON.parse(error.error) : JSON.parse(`{ "Message": "${error.error.message}" }`));
    }
}
